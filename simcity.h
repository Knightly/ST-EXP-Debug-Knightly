...
public:
	struct factory_entry_t
	{
		...
		sint32 demand;		// shifted by DEMAND_BITS; amount demanded by the factory
		sint32 supply;		// shifted by DEMAND_BITS; portion of demand that the city can supply; supply <= demand
		sint32 remaining;	// shifted by DEMAND_BITS; portion of supply which has not realised yet; remaining <= supply
		sint32 departed;	// amount which has departed with a valid route

		factory_entry_t() : factory(NULL), demand(0), supply(0), remaining(0), departed(0) { }
		factory_entry_t(fabrik_t *_factory) : factory(_factory), demand(0), supply(0), remaining(0), departed(0) { }
		factory_entry_t(fabrik_t *_factory, sint32 _demand) : factory(_factory), demand(_demand), supply(_demand), remaining(_demand), departed(0) { }

		bool operator == (const factory_entry_t &other) const { return ( this->factory==other.factory ); }
		void new_month() { remaining = supply = demand; departed = 0; }
	...
	struct factory_set_t
	{
		...
		sint32 total_remaining;		// shifted by DEMAND_BITS
		sint32 total_generated;
		uint32 generation_ratio;	// (demand * 100 / total) shifted by RATIO_BITS
		bool ratio_stale;
		...
		const factory_entry_t* get_entry(fabrik_t *const factory) const;
...
public:
	...
	factory_set_t& get_target_factories_for_pax() { return target_factories_pax; }
	factory_set_t& get_target_factories_for_mail() { return target_factories_mail; }
