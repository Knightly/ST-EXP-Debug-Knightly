...
const stadt_t::factory_entry_t* stadt_t::factory_set_t::get_entry(fabrik_t *const factory) const
{
...
void stadt_t::factory_set_t::update_factory(fabrik_t *const factory, const sint32 demand)
{
	if(  entries.is_contained( factory_entry_t(factory) )  ) {
		...
		// new target factory
		entries.append( factory_entry_t(factory, demand) );		// the new entry's supply and remaining values are also initialized with the demand value
		total_demand += demand;
		total_remaining += demand;
	}
...
void stadt_t::factory_set_t::recalc_generation_ratio(const sint32 default_percent, const sint64 *city_stats, const int stats_count, const int stat_type)
{
	...
	// ratio formula -> ((demand * 100) / total) shifted by RATIO_BITS
	if(  total_demand==0  ) {
		...
		// ratio of total factory pax demand (plus allowances for rounding up) to previous months' average generated pax (less 6.25% or 1/16 allowance for fluctuation), capped by default ratio
		const sint64 default_ratio = (sint64)default_percent << RATIO_BITS;
		const sint64 demand_ratio = ((sint64)(((total_demand>>DEMAND_BITS)+(sint32)entries.get_count())*100)<<RATIO_BITS) / (average_generated-(average_generated>>4)+1);
		generation_ratio = (uint32)( default_ratio<demand_ratio ? default_ratio : demand_ratio );
	}
	...
	if(  welt->get_einstellungen()->get_factory_enforce_demand()  &&  (generation_ratio>>RATIO_BITS)==default_percent  &&  average_generated>0  &&  total_demand>0  ) {
		const sint64 supply_promille = ( ( (average_generated << 10) * (sint64)default_percent ) / 100 ) / (sint64)(total_demand >> DEMAND_BITS);
		if(  supply_promille<1024  ) {
			// expected supply is really smaller than demand
			for(  uint32 e=0;  e<entries.get_count();  ++e  ) {
				factory_entry_t &entry = entries[e];
				const sint32 new_supply = (sint32)( ( (sint64)entry.demand * supply_promille ) >> 10 );
	...
	// expected supply is unknown or sufficient to fulfil demand
	for(  uint32 e=0;  e<entries.get_count();  ++e  ) {
		factory_entry_t &entry = entries[e];
		const sint32 delta_supply = entry.demand - entry.supply;
		if(  delta_supply==0  ) {
			continue;
		}
		else if(  delta_supply>0  ||  (entry.remaining+delta_supply)>=0  ) {
...
void stadt_t::factory_set_t::rdwr(loadsave_t *file)
{
	if(  file->get_version()>=110005  ) {
		uint32 entry_count;
		if(  file->is_saving()  ) {
			entry_count = entries.get_count();
		}
		file->rdwr_long(entry_count);
		if(  file->is_loading()  ) {
			entries.init_with_new_elements( entry_count );
		}
		for(  uint32 e=0;  e<entry_count;  ++e  ) {
			entries[e].rdwr( file );
			if(  file->is_loading()  ) {
				total_demand += entries[e].demand;
				total_remaining += entries[e].remaining;
			}
		}
...
void stadt_t::step(long delta_t)
{
	...
	// recalculate factory going percents where necessary
		...
		for(  int pax_routed=0, pax_left_to_do=0;  pax_routed<num_pax;  pax_routed+=pax_left_to_do  ) {
			...
			if(  factory_entry  ) {
				if(  welt->get_einstellungen()->get_factory_enforce_demand()  ) {
					// ensure no more than remaining amount rounded up
					pax_left_to_do = min( pax_left_to_do, ( (factory_entry->remaining + ((1 << DEMAND_BITS) - 1)) >> DEMAND_BITS ) );
					const sint32 reduction = ( factory_entry->remaining<=(pax_left_to_do<<DEMAND_BITS) ? factory_entry->remaining : (pax_left_to_do<<DEMAND_BITS) );
					factory_entry->remaining -= reduction;
					target_factories.total_remaining -= reduction;
				}
...
// DBG_MESSAGE("stadt_t::step_passagiere()", "No stop near dest (%d, %d)", ziel.x, ziel.y);
			...
			if (can_walk_ziel) {
				if(  factory_entry  ) {
					...
					factory_entry->departed += pax_left_to_do;
					if(  wtyp==warenbauer_t::passagiere  ) {
						factory_entry->factory->book_pax_departed(pax_left_to_do);
					}
					else {
						factory_entry->factory->book_mail_departed(pax_left_to_do);
					}
			...
			if(  route_result == haltestelle_t::ROUTE_OK  ) {
				...
				if(  factory_entry  ) {
					factory_entry->departed += pax.menge;
					if(  wtyp==warenbauer_t::passagiere  ) {
						factory_entry->factory->book_pax_departed( pax.menge );
					}
					else {
						factory_entry->factory->book_mail_departed( pax.menge );
					}
				}
		...
		if(  factory_entry  ) {
			...
			if(  welt->get_einstellungen()->get_factory_enforce_demand()  ) {
				// ensure no more than remaining amount rounded up
				amount = min( amount, ( (factory_entry->remaining + ((1 << DEMAND_BITS) - 1)) >> DEMAND_BITS ) );
				const sint32 reduction = ( factory_entry->remaining<=(amount<<DEMAND_BITS) ? factory_entry->remaining : (amount<<DEMAND_BITS) );
				factory_entry->remaining -= reduction;
				target_factories.total_remaining -= reduction;
			}
