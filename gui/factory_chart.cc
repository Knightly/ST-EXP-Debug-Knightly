...
static sint64 convert_goods(sint64 value) { return ( value >> fabrik_t::precision_bits ); }
static sint64 convert_power(sint64 value) { return ( value >> POWER_TO_MW ); }
static sint64 convert_boost(sint64 value) { return ( (value * 100) >> DEFAULT_PRODUCTION_FACTOR_BITS ); }
...
{
	"Production :", "Power :", "Boost :", "Max Boost :", "Demand :", "Passenger :", "Mail :"
};
	...
	gui_frame_t( _factory->get_name(), _factory->get_besitzer() ), factory(_factory), goods_buttons(NULL), goods_labels(NULL), goods_button_count(0), goods_label_count(0)
{
	...
	if(  input_count>0  ) {
		...
		++goods_label_count;
		const vector_tpl<ware_production_t> &input = factory->get_eingang();
	...
	if(  output_count>0  ) {
		...
		goods_cont.add_komponente( goods_labels + goods_label_count );
		++goods_label_count;
		const vector_tpl<ware_production_t> &output = factory->get_ausgang();
...
void factory_chart_t::zeichnen(koord pos, koord gr)
{
	...
	prod_ref_line_data[FAB_REF_DEMAND_ELECTRIC] = ( factory->get_besch()->is_electricity_producer() ? factory->get_scaled_electric_amount() : 0 );
